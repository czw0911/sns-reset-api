...
type Comments struct {
	  ...
	  CommentID string //评论id = 谈论id _ id
...
func (u *Comments) initCommentDBAndTable() {
		...
		u.tableName = fmt.Sprintf("comment_%d_%s",u.ActivityID,u.YearAndMonth)	
}
...
func (u *Comments) Add() (bool,error) {
	...
	) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='谈论内容的评论月表';
	...
	_,err = u.dbConn.Raw("INSERT INTO "+ u.tableName +"(UID,TalkID,Contents,PostTime) VALUES(?,?,?,?)",
	u.UID,u.TalkID,u.Contents,u.PostTime).Exec()
	if err != nil {
		return false, err
	}
	return true, nil
...
func (u *Comments) PageUp() []orm.Params {
	u.initCommentDBAndTable()
	sqlStr := fmt.Sprintf("SELECT ID,  UID,TalkID,Contents,PostTime  FROM %s WHERE ID > ? AND TalkID = ?  AND IsShow = 0 ORDER BY ID ASC LIMIT %d",u.tableName,XYLibs.TABLE_LIMIT_NUM)
	var table []orm.Params
	u.dbConn.Raw(sqlStr,u.MaxID,u.TalkID).Values(&table)
	return table
...
func (u *Comments) PageDown() []orm.Params {
	u.initCommentDBAndTable()
	sqlStr := fmt.Sprintf("SELECT ID, UID,TalkID,Contents,PostTime  FROM %s WHERE ID < ?  AND TalkID = ?  AND IsShow = 0  ORDER BY ID DESC LIMIT %d",u.tableName,XYLibs.TABLE_LIMIT_NUM)
...
func (u *Comments) PageEnd() []orm.Params {
	u.initCommentDBAndTable()
	sqlStr := fmt.Sprintf("SELECT ID, UID,TalkID,Contents,PostTime  FROM %s  WHERE TalkID = ?  AND IsShow = 0  ORDER BY ID ASC LIMIT %d",u.tableName,XYLibs.TABLE_LIMIT_NUM)
	var table []orm.Params
...
func (u *Comments) PageFirst() []orm.Params {
	u.initCommentDBAndTable()
	sqlStr := fmt.Sprintf("SELECT ID, UID,TalkID,Contents,PostTime FROM %s  WHERE TalkID = ?  AND IsShow = 0   ORDER BY ID Desc LIMIT %d",u.tableName,XYLibs.TABLE_LIMIT_NUM)
	var table []orm.Params
	u.dbConn.Raw(sqlStr,u.TalkID).Values(&table)
	return table
}
