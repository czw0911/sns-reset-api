...
type mysqlConf struct {
	...
	UserHashTableNum	int	`json:"user_table_hash_num"`
	UserDBList []mysqlDns `json:"user_db_list"`
}
...
func (c *mySQLHash) GetConsistentByUID(uid uint32) (string,error){
...
//取模hash
func (c *mySQLHash) GetModHashByUID(dbType int8,uid string) int {
	
	f := fnv.New32a()
	io.WriteString(f,uid)
	u,err := strconv.Atoi(fmt.Sprintf("%x",f.Sum(nil)))
	if err != nil {
		println(err)
		return 0
	}
	if dbType == 1 {
		return u % c.DBCountMaste
	}
	return u % c.DBCountSlave
