...
func (u *UserDetailInfo) Reg() (bool, error) {
	u.initDBAndTable()
	sqlStr := fmt.Sprintf("REPLACE INTO %s(NickName,Avatar,Thumbnail,UID,HomeProvinceID,HomeCityID,HomeDistrictID,LivingProvinceID,LivingCityID,LivingDistrictID,HomeVoice,VoiceLen) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", u.tableName)
	_, err := u.dbConn.Raw(sqlStr, u.NickName, u.Avatar, u.Thumbnail, u.UID, u.HomeProvinceID, u.HomeCityID, u.HomeDistrictID, u.LivingProvinceID, u.LivingCityID, u.LivingDistrictID, u.HomeVoice, u.VoiceLen).Exec()
	if err != nil {
		return false, err
	}
	return true, nil
