...
func (u *UpdateUserInfoController) Post() {
	...
	loginToken := GetLoginToken(detailDB.UID)
	if loginToken == "" {
	...
	auth := XYLibs.CheckLoginSign(u.Ctx,sign, loginToken,[]string{"Sign","Avatar"})
	...
	if detailDB.NickName != "" {		
		nickNameTmp := strings.Replace(detailDB.NickName," ","",-1)
		if  nickNameTmp == "乡音团队"  ||  nickNameTmp == "乡仔"  || nickNameTmp == "乡妹" {
			u.Data["json"] = XYLibs.RespStateCode["user_nickname_is_exist"]
			u.ServeJson()
			return
		}
