...
const DYNAMIC_TABLE_SELECT_COLUMN_NAME = " ID,UID,DynamicID,DynamicContent,ViewNUM,CommentNum,ForwardNum,GoodNUM,Images,Voices,VoiceLen,PostTime,LastComment " 
...
//获取指定动态信息
func (u *DynamicList) GetDynamicInfo()[]orm.Params {
	u.initDynamicDBAndTable()
	u.dbConn =  ConnSlaveDB(u.HomeProvinceID)
	sqlStr := fmt.Sprintf("SELECT %s FROM %s  WHERE DynamicID = ?  LIMIT 1",DYNAMIC_TABLE_SELECT_COLUMN_NAME,u.tableName)
...
//删除
func (u *DynamicList) ClickDelete() (bool,error) {
	p := u.ParseDynamicID()
	if !p {
		return false,errors.New("动态id格式错误")
	}
	...
	sqlStr := fmt.Sprintf("UPDATE %s SET IsShow = 1  WHERE DynamicID = ? AND UID = ?",u.tableName)
	_, err := u.dbConn.Raw(sqlStr,u.DynamicID,u.UID).Exec()
	if err != nil {
		return false, err
	}
	return true, nil
...
func (u *DynamicList) ClickComment() (bool,error) {
	...
	sqlStr := fmt.Sprintf("UPDATE %s SET CommentNUM = CommentNUM + 1 , LastComment = ?  WHERE DynamicID = ?",u.tableName)
	_, err := u.dbConn.Raw(sqlStr,u.LastComment,u.DynamicID).Exec()
	if err != nil {
		return false, err
	}
	return true, nil
...
//评论删除
func (u *DynamicList) ClickDelComment() (bool,error) {
	p := u.ParseDynamicID()
	if !p {
		return false,errors.New("动态id格式错误")
	}
...
func (u *DynamicList) PageUp() []orm.Params {
	...
	sqlStr := fmt.Sprintf("SELECT  %s FROM %s WHERE ID > ? AND IsShow = 0  %s ORDER BY ID ASC LIMIT %d",DYNAMIC_TABLE_SELECT_COLUMN_NAME,u.tableName,strWhere,XYLibs.TABLE_LIMIT_NUM)
	var table []orm.Params
...
func (u *DynamicList) PageDown() []orm.Params {
	...
	sqlStr := ""
	if u.DataIndex > 0 {
		sqlStr = fmt.Sprintf("SELECT %s FROM %s WHERE ID < ? AND IsShow = 0  %s ORDER BY ID DESC LIMIT %d",DYNAMIC_TABLE_SELECT_COLUMN_NAME,u.tableName,strWhere,XYLibs.TABLE_LIMIT_NUM)
	}else{
		sqlStr = fmt.Sprintf("SELECT %s FROM %s WHERE ID > ? AND IsShow = 0  %s ORDER BY ID DESC LIMIT %d",DYNAMIC_TABLE_SELECT_COLUMN_NAME,u.tableName,strWhere,XYLibs.TABLE_LIMIT_NUM)
	}
	var table []orm.Params
...
func (u *DynamicList) PageEnd() []orm.Params {
	...
	sqlStr := fmt.Sprintf("SELECT %s FROM %s  WHERE  IsShow = 0  %s ORDER BY ID ASC LIMIT %d",DYNAMIC_TABLE_SELECT_COLUMN_NAME,u.tableName,strWhere,XYLibs.TABLE_LIMIT_NUM)
	var table []orm.Params
...
func (u *DynamicList) PageFirst() []orm.Params {
	...
	sqlStr := fmt.Sprintf("SELECT %s FROM %s WHERE  IsShow = 0   %s ORDER BY ID DESC LIMIT %d",DYNAMIC_TABLE_SELECT_COLUMN_NAME,u.tableName,strWhere,XYLibs.TABLE_LIMIT_NUM)
	var table []orm.Params
	u.dbConn.Raw(sqlStr).Values(&table)
	return table
}
