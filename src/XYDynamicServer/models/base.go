...
func ConnSlaveDB(HomeProvinceID int) orm.Ormer {
	d := orm.NewOrm()
	index,err := libs.SalveDB.GetConsistentSalveDB(HomeProvinceID)
	if err != nil {
		beego.Error(err)
	}
	d.Using(index)
...
func ConnMasterDB(HomeProvinceID int) orm.Ormer {
	d := orm.NewOrm()
	index,err := libs.MasteDB.GetConsistentMasterDB(HomeProvinceID)
	if err != nil {
		beego.Error(err)
	}
	d.Using(index)
